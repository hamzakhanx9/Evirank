...
def run_bm25_test(claim_index=-1, k=5, dataset=None, verbose=True):
    if dataset is None:
    ...
    if claim_index == -1:
        ...
        for idx, _ in enumerate(dataset[:250]):
            ...
            precision_at_k, recall_at_k, ap_score = run_bm25_test(idx, k, dataset, verbose=False)
        ...
        with open(f"{results_dir}bm25_avg.txt", "w") as f:
        ...
        print("\nResults saved to results/bm25_avg.txt")
        return
    ...
    top_k_docs = [doc for doc, _ in top_docs[:k]]
