...
def run_hybrid_test(claim_index=-1, k=5, dataset=None, verbose=True):
    if dataset is None:
    ...
    if claim_index == -1:
        ...
        for idx, _ in enumerate(dataset[:250]):
            ...
            precision_at_k, recall_at_k, ap_score = run_hybrid_test(idx, k, dataset, verbose=False)
        ...
        with open(f"{results_dir}hybrid_avg.txt", "w") as f:
        ...
        print("\nResults saved to results/hybrid_avg.txt")
        return
    ...
    bi_reranked = rerank(claim, [doc for doc, _ in top_docs])

    top_25 = [doc for doc, _ in bi_reranked[:25]]
    cross_encoder = CrossEncoderReranker()
    reranked = cross_encoder.rerank(claim, top_25, top_n=k)

    reranked_docs = [doc for doc, _ in reranked]

    if verbose:
        print(f"\nHybrid (Bi-Encoder + Cross-Encoder) Reranked Results (Top {k}):")
